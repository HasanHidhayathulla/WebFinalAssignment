 ...
 */
function checkMemberIdExists($conn, $member_id, $exclude_id = null) {
    $member_id = trim($member_id);
    ...
    if (!$stmt) {
        return ['exists' => false, 'error' => 'Database error: ' . $conn->error];
    }
 ...
 */
function validateMemberData($data, $conn, $mode = 'create', $exclude_member_id = null) {
    $errors = [];
    ...
    if (!$member_id_validation['valid']) {
        ...
        if ($check_duplicate) {
            $duplicate_check = checkMemberIdExists($conn, $data['member_id'], $exclude_member_id);
